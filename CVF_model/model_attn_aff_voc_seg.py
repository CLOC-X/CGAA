...
class CVF(nn.Module):
    ...
    def forward(self, img, img_names='2007_000032', mode='train',cam_only=False):
        b, c, h, w = img.shape
        self.iter_num += 1
        with torch.no_grad():
            attn_weights, cams, patch_attn, x_patch = self.mct(img,return_att=True)  # #  attn_weights 12 * B * H * N * N
