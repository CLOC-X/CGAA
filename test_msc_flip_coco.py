...
parser.add_argument("--config",
                    ...
                    type=str,
                    help="config")
parser.add_argument("--work_dir", default="results_coco", type=str, help="work_dir")
...
parser.add_argument("--model_path", default="xxxx/CVF_model_iter_80000.pth",
...
def validate(model, dataset, test_scales=None):
    ...
    for idx, data in tqdm(enumerate(data_loader), total=len(data_loader), ncols=100, ascii=" >="):
        ...
        segs_cat, cam, attn_loss,_ = model(inputs_cat, names, mode = 'val')
        ...
        for s in test_scales:
            if s != 1.0:
                ...
                segs_cat, cam_cat, attn_loss,_ = model(inputs_cat, names, mode='val')
...
def crf_proc(config):
    ...
    images_path = os.path.join(config.dataset.root_dir, 'val2014',)
    labels_path = os.path.join(config.dataset.root_dir, 'mask/val2014')
