...
def diff_weight_seg(seg_attn, attn_weight):
...
def perform_single_voc_cam(img_path, image, image_features, attn_weight_list, seg_attn, patch_attn, bg_text_features,
    ...
    for idx, label in enumerate(label_list):
        ...
        if idx == 0:
            if require_seg_trans == True:
                attn_weight = torch.cat([attn_weight_list[:, 1:, 1:], attn_weight_last[:, 1:, 1:],patch_attn], dim=0)
                attn_weight = attn_weight[-7:] #-8
                attn_weight = diff_weight_seg(seg_attn, attn_weight)
            else:
                attn_weight = torch.cat([attn_weight_list[:, 1:, 1:], attn_weight_last[:, 1:, 1:],patch_attn], dim=0)
                attn_weight = attn_weight[-9:]
    ...
    if mode == 'train':
        return cam_refined_list,trans_mat, keys, w, h
    else:
        return cam_refined_list,trans_mat, keys, ori_width, ori_height
...
def perform_single_coco_cam(img_path, image, image_features, attn_weight_list, seg_attn, patch_attn, bg_text_features,
