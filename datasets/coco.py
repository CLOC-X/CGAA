...
class CocoDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.stage == "train":
            ...
            label_dir = os.path.join(self.label_dir, 'COCO_train2014_'+_img_name+'.png')
            label = np.asarray(Image.open(label_dir))
            # label = np.asarray(imageio.imread(label_dir))

        elif self.stage == "val":
            _img_name = _img_name[13:]
            label_dir = os.path.join(self.label_dir, 'COCO_val2014_'+_img_name+'.png')
            label = np.asarray(Image.open(label_dir))
            # label = np.asarray(imageio.imread(label_dir))
