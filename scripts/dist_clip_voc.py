...
from utils.camutils import cams_to_affinity_label, refine_pgt_from_cam
from utils.optimizer import PolyWarmupAdamW
from utils.AverageMeter import AverageMeter, cal_eta, format_tabs, setup_logger
from CVF_model.model_attn_aff_voc import CVF
...
parser.add_argument("--w_attn", default=0.1, type=float, help="w_attn")
parser.add_argument("--w_pat", default=0.1, type=float, help="w_pat")
...
def validate(model=None, data_loader=None, cfg=None):
    ...
    for _, data in tqdm(enumerate(data_loader),
        ...
        segs, cam, attn_loss,_,_= model(inputs,name, 'val')
    ...
    tab_results = format_tabs([cam_score, seg_score], name_list=["CAM",  "Seg_Pred"], cat_list=voc.class_list)

    return seg_score, cam_score, tab_results
...
def train(cfg):
    ...
    param_groups = CVF_model.get_param_groups()
    CVF_model.cuda()
    ...
    optimizer = PolyWarmupAdamW(
        params=[
            ...
            },
            {
                ...
                "lr": cfg.optimizer.learning_rate*2,
                "weight_decay": cfg.optimizer.weight_decay,
            },
            {
    ...
    for n_iter in range(cfg.train.max_iters):
        ...
        segs, cam, attn_pred, x_patch_logits, refine_cams= CVF_model(inputs.cuda(), img_name,n_iter=n_iter)
        pseudo_label = refine_pgt_from_cam(inputs,refine_cams,cls_labels, cfg.dataset.num_classes-1,img_name)
        combined_pseudo_label = torch.max(pseudo_label, cam)
        segs = F.interpolate(segs, size=combined_pseudo_label.shape[1:], mode='bilinear', align_corners=False)
        seg_loss = get_seg_loss(segs, combined_pseudo_label.type(torch.long), ignore_index=255)
        ...
        pat_loss = F.multilabel_soft_margin_loss(x_patch_logits, cls_labels)
        loss = 1 * seg_loss + w_attn * attn_loss + w_pat * pat_loss



        avg_meter.add({'seg_loss': seg_loss.item(), 'attn_loss': attn_loss.item(),'pat_loss': pat_loss.item()})
        ...
        if (n_iter + 1) % cfg.train.eval_iters == 0:
            ckpt_name = os.path.join(cfg.work_dir.ckpt_dir, "CVF_model_iter_%d.pth"%(n_iter+1))
            logging.info('Validating...')
            if (n_iter + 1) > 26000:
                torch.save(CVF_model.state_dict(), ckpt_name)
            seg_score, cam_score, tab_results = validate(model=CVF_model, data_loader=val_loader, cfg=cfg)
...
if __name__ == "__main__":
    ...
    cfg.work_dir.tb_logger_dir = os.path.join(cfg.work_dir.dir, "tb_logger_dir")
