...
import CVF_model.MCT_ViT.models
...
parser.add_argument("--model_path", default="XX/CVF_model_iter_30000.pth", type=str, help="model_path")
parser.add_argument("--mct_path", default='XX/checkpoint.pth', type=str, help="mct_path")
...
def validate(model, dataset, test_scales=None):
    ...
    for idx, data in tqdm(enumerate(data_loader), total=len(data_loader), ncols=100, ascii=" >="):
        ...
        checkpoint = torch.load(args.mct_path, map_location='cpu')
        mct_model.load_state_dict(checkpoint['model'])
        segs_list = []
        inputs_cat = torch.cat([inputs, inputs.flip(-1)], dim=0)
        segs_cat, cam, attn_loss = model(inputs_cat, mct_model.cuda(),cls_labels, mode = 'val')
        ...
        for s in test_scales:
            if s != 1.0:
                ...
                segs_cat, cam_cat, attn_loss = model(inputs_cat,  mct_model.cuda(),cls_labels, mode='val')
...
def main(cfg):
    ...
    CVF_model = CVF(num_classes=cfg.dataset.num_classes,
    ...
    CVF_model.load_state_dict(state_dict=trained_state_dict, strict=False)
    CVF_model.eval()

    gts, preds, msc_preds, cams, preds_hist, msc_preds_hist, cams_hist = validate(model=CVF_model, dataset=val_dataset, test_scales=[1, 0.75])
