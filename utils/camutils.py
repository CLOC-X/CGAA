...
def _refine_cams(ref_mod, images, cams, valid_key):
    ...
    refined_cams = F.interpolate(refined_cams, size=(images.shape[-2],images.shape[-1]), mode="bilinear", align_corners=False)
    refined_label = refined_cams.argmax(dim=1)
    refined_label = valid_key[refined_label]

    return refined_label.squeeze(0)
def _crf_with_alpha(cam_dict, alpha, orig_img):
    cam_label = np.array(list(cam_dict.values()))
    bg_score = np.power(1 - np.max(cam_label, axis=0, keepdims=True), alpha)
    cam_label = np.concatenate((bg_score, cam_label), axis=0)
    crf_score = crf_inference(orig_img, cam_label, labels=cam_label.shape[0])
    n_crf_al = dict()
    n_crf_al[0] = crf_score[0]
    for i, key in enumerate(cam_dict.keys()):
        n_crf_al[key + 1] = crf_score[i + 1]
    cam_label = np.array(list(n_crf_al.values()))
    bg_score = np.power(1 - np.max(cam_label, axis=0, keepdims=True), alpha)
    cam_label = np.concatenate((bg_score, cam_label), axis=0)
    keys = np.array(list(n_crf_al.keys()))
    valid_key = np.pad(keys + 1, (1, 0), mode='constant')
    valid_key = torch.from_numpy(valid_key).cuda()
    return cam_label, valid_key
def refine_pgt_from_cam(img,cams, cls_label, num_classes, img_names,ignore_index=255):
    ...
    for i, img_name in enumerate(img_names):
        cam = cams[i] * cls_label[i].view(num_classes, 1, 1)
        if (cls_label[i].sum()) > 0:
            cam_dict = {}    
            for cls_ind in range(num_classes):
                if cls_label[i,cls_ind]>0:
                    # cls_score = format(output[b, cls_ind].cpu().numpy(), '.3f')
                    cls_attention = cam[cls_ind,:,:]
            ...
            cam_label = np.array(list(cam_dict.values()))
            cam_label = torch.from_numpy(cam_label).cuda()
            bg_score = torch.pow(1 - torch.max(cam_label, dim=0, keepdims=True)[0], 1).cuda()
            cam_label = torch.cat([bg_score, cam_label], dim=0).cuda()
            keys = np.array(list(cam_dict.keys()))
            valid_key = np.pad(keys + 1, (1, 0), mode='constant')
            valid_key = torch.from_numpy(valid_key).cuda()
            refined_label = _refine_cams(par, img[i].cuda(), cam_label, valid_key) 

     
            cam_list.append(refined_label)
    all_cam_labels = torch.stack(cam_list, dim=0)
    return  all_cam_labels 
